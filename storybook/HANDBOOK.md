...
**招词表**（2026-09-16 的恐龙十本实测，每个都害得那页返工一次）。这些词本身没错，
错在它们在模型心里有个更强的默认形象，会盖掉你后面写的所有限定：

| 写了什么 | 画出来是什么 | 改成怎么写 |
|---|---|---|
| `babies`（小恐龙） | 人类婴儿：光头人脸、五指手、还有肚脐 | `hatchlings with scaly skin, beaked snouts and stubby tails` |
| `dolphin`（说某海爬像海豚） | 一只现代宽吻海豚 | 只说形状：`narrow pointed snout, four paddle flippers, upright tail fin`（海豚只有两片胸鳍） |
| `triceratops skull` | 满嘴牙的霸王龙头骨，没角没盾 | 不写物种名，写形状：`one short horn on its nose, one long horn above each eye, a wide bony shield fanning out behind its head` |
| `footprints`（恐龙足迹） | 人的脚印：五个脚趾、脚跟、足弓 | 这页直接改判给程序画 |
| `tooth`（恐龙的牙） | 牙医海报上那种带两条牙根的臼齿 | 改判给程序，或写 `cone-shaped, a row of tiny saw teeth down its edge` |
...
或者让草石、泥土、天空填满画面，不给可写字的平面留位置。

### 4.2h 他出镜的页：脸要大、要正、画风要留得住笔触 ★★

2026-09-16 用户第三次说「人物不像他」。量了第六批 18 页他出镜的页，
数据把原因说得很死：

| 测到的 | 数字 |
|---|---|
| 脸占画面不到 3% | **13 / 18**（1024 的图上脸宽 120–200px，留给五官几十个像素） |
| 眼睛是纯黑椭圆、没有眼白和瞳孔 | **15 / 18** |
| 脸做过明暗塑形 | **0 页**（所谓质感全是画风的纸纹布纹涂满全图，不是脸的结构） |
| 接近正脸 | 只有 3 页；12 页是仰头四分之三，下巴抬起把脸压扁 |

**这不是参考照片的问题。** 脸小于 3% 加黑点当眼睛这两条摆在那儿，
照片给得再准也不可能认出是谁——画出来的是一副「绘本通用娃娃脸」模板：
同一对黑豆眼、腮红圆圈、平涂肤色，换画风只换了背景和笔触材质，没换脸的画法。

三条规则，他出镜的页每页都要套：

1. **脸要大。** 全身站姿改成 `Waist-up view of the little boy`（半身）或更近。
   脸占比从不到 3% 提到 12–30%，五官才有还原的余地。
2. **明写脸的朝向。** `his face large in the frame and turned towards the viewer`。
   不写就由模型选，它一选就是仰头四分之三，甚至全侧面。
3. **明写眼睛的结构。** `both eyes clearly drawn with white, a dark round iris and a
   small bright highlight`。第六批唯一画对眼睛的那一页，恰好也是看着最像他的一页。

**还有一条更根本的：平涂画风和「像某个具体的孩子」是冲突的。**
`bold flat graphic art with thick clean outlines and flat fills` 这类画风，
脸就是粗轮廓加几个色块，把脸转正、放大都救不回来——只能换画风。
《霸王龙的牙齿》换成「水彩 + 铅笔线」之后，脸占比提到 12–15%，
**眼睛自己长出了瞳孔和高光**，眉毛、鼻子、耳朵也都有了，第 3 条规则甚至没写就自动满足。
反过来，本来就保留笔触的画风（彩铅、水彩、色粉、油画棒、麻布）不必换，
只改构图和朝向就够——这一条省掉了九本整本重画。

顺带一个这次新踩的：**换画风之后，道具也会被重新理解。** 同一句
`one huge fossil tooth`，平涂画风下画的是牙，水彩画风下画成了一根腿骨。
解法还是 4.2f 那条——不靠名词，写死形状（光滑的锥、底宽圆、顶上一个尖）。
改道具的时候**人物那部分一个字都别动**，免得把已经对了的东西改坏。

### 4.2g 程序画「有形状的东西」：剪影、占位、贴边 ★

前几批程序画的都是方框、圆、箭头、横条。2026-09-16 的恐龙十本第一次要画**认得出的活物**
（兽脚类、蜥脚类、三角龙、鸡、猫、翼龙、大象），56 页里子代理打回 22 页。错法很集中，
四条都能直接变成写代码时的规矩：

**一、一个物件就是一条闭合轮廓，不要拼零件。** 兽脚类第一版把掌、身、腿拆成几块圆角矩形
和粗线段，出来的是一块平板加两条绞在一起的腿；蜥脚类是只肥鸭。改法是在单位坐标里
（体长 1.0、原点在髋）设计一次点列，头、颈、躯干、尾各留出明确的段落，尾巴一路收细。
腿是唯一的例外——它必须单独画，但也得用**三段多边形**（大腿/小腿/脚），并且
**远侧那条先画、填深色，近侧的后画**，前后才分得开。

**二、每个剪影都要记住它的占位数，布局拿数算，不要凭感觉。**

| 图元 | 站立高度 | 脚底 |
|---|---|---|
| 兽脚类 | 0.66 × 体长 | cy + 0.33 × 体长 |
| 蜥脚类 | 0.76 × 体长 | cy + 0.32 × 体长 |
| 汽车 | 顶棚比车身还高出 1.25 倍车高 | — |

不查这张表就会出这些事：恐龙整只悬空 190px（旁白写的是「站在地上」）、脚穿到地线下面、
三角龙站进了汽车的顶棚里、蜥脚类的腿插进下面那排公交车。**并排比大小的两个东西，
要按各自的实际占位错开，不是按中心线对齐。**

**三、沿轮廓走的粗笔画必须向体内让，而且只走中间段。** 「背上深、肚子浅」那页，深色带
从吻画到尾尖、笔宽 34px，结果头被整个盖住、两端戳到身体外面，读成一条搭在恐龙身上的
独立带子。改成只走肩→尾根、整条向体内让 16px 才对。同理，**往形状里撒点要用重心坐标**
（叶子堆那页有七八片浮在堆外面）。

**四、「盖住」这个意思，别真盖住。** 「泥沙一层层把恐龙盖住」那页，把层压在恐龙身上之后
头就没了，反而认不出是恐龙。改成恐龙完整可见、泥沙在上方、几支箭头表示正往下盖。
**旁白要的是那个意思，不是那个动作。**

最后一条老规矩在这批又验了一次：**旁白点名的参照物，画面里必须真有那个东西。**
「和一个足球差不多大」只画了两个蛋、「从和猫一样大长到比车还大」既没有猫也没有车——
这种错自己看图很难发现，因为知道它该是什么；交给子代理对着旁白核就一抓一个准。
所以每个页面函数都要写一句 docstring 说明这页画什么——它不是注释，是校验用的。

### 4.2i 抠素材：别信 floodfill，外框错了整页比例都错 ★★

素材拼合这条路成立的前提是**外框（bbox）等于物体本身**——`place(h=...)` 拿外框算缩放，
外框虚高一成，物体就小一成。2026-09-16 发现前面所有对比页的比例都是错的，原因在抠图：

**模型给的「纯色底」不是纯色。** 剑龙那张量出来：背景占 76% 的像素，和边框中位色的距离
12% 在 10 以内、63% 在 30 以内、76% 在 45 以内——也就是**纸纹本身就铺满 0 到 45 的色差**，
里头总有几条像素超过 thresh，把背景切成互不连通的几块。`ImageDraw.floodfill` 是按连通性
走的，只吃得掉和画布边缘连通的那一块，**剩下的奶油色岛屿全留在图里**。那张的外框因此是
920×825，物体其实只有 920×556，整整虚高三成。

抠图改成三步，都不靠连通性：

1. **和边框中位色的距离 ≤ tol 的，是背景。**（边框中位色比角点稳：量了一圈，边框六像素内
   的偏差 ≤ 6，而角点偶尔正好压在物体上。）
2. **色相和背景几乎一样、只是暗了一点点的，是脚下的投影，也当背景。** 投影的色相差 5.8、
   背景 1.6、绿肚皮 12.2、橙背板 47——投影和真颜料分得开。加一条「亮度还在背景七成以上」
   把深色描边排除掉，描边才留得住。
3. **剩下的填内洞（`binary_fill_holes`）。** 这一步保住浅色的蛋壳、小鸡、骨头：它们外面有
   一圈闭合的深色描边，描边围起来的都算物体。没有这一步，一只奶油色的小鸡会被抠成空壳。

最后丢掉面积不到最大块 2% 的碎块（纸纹残渣），蒙版向内收 2px，裁到外框。

**更省事的一半在提示里。** 水彩风的素材脚下常自带一摊奶油色的地面色块——那是真颜料，
色相和背景不一样，任何颜色规则都分不掉。素材提示里直接写死：
「The object floats on this flat colour: it casts no shadow and rests on no ground, and there is
no darker patch, wash, puddle, base, line or platform of any kind underneath it.」
反向提示加 `shadow, cast shadow, ground, floor, base, pedestal, platform`。

**怎么发现的：把抠出来的图贴到品红底上看一眼。** 白底上看不出残留，品红底上一目了然。
这一眼值得在每批素材出完后花——量外框长宽比也行：一只侧面的剑龙长宽比该在 1.6 上下，
量出来 1.12 就是抠坏了。
